...
public class NewBlockchain : MonoBehaviour
{
    ...
    public string dbUrl_mainnet = "https://metx-botshooter-server.vercel.app/api/v1";
    public string gameId = "E5C00C161D1698C1";
    public string auth;
    ...
    private void PostEventScore(int amount)
    {
        if (idb.node() == "testnet")
        {
            StartCoroutine(EventScore(dbUrl_testnet, idb.bearerTestnet(), amount));
        }
        else if (idb.node() == "mainnet")
        {
            StartCoroutine(EventScore(dbUrl_mainnet, idb.bearerMainnet(), amount));
        }
         
    }

    private IEnumerator EventScore(string baseUrl, string bearer, int tokensClaimed)
    {
        int amount = tokensClaimed;
        var cert = new ForceAcceptAll();
        string url = $"{baseUrl}/api/v1/events/score?score={amount}&auth={idb.auth()}&gameId={gameId}";
